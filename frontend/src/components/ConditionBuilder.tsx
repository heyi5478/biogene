...
export function ConditionBuilder({
  ...
  const addRow = () => {
    onConditionsChange([
      ...conditions,
      { id: newConditionId(), moduleId: '', fieldId: '', operator: 'eq', value: '', value2: '' },
    ]);
  };
  ...
  const getFieldDef = (moduleId: ModuleId | '', fieldId: string): FieldDefinition | undefined => {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="space-y-1">
          {CONDITION_TEMPLATES.map((t) => (
            <button
              ...
              className="flex items-start gap-2 w-full text-left px-2.5 py-2 rounded-md text-xs border border-border hover:bg-accent/50 transition-colors"
            >
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-2">
          {conditions.map((row, idx) => {
            const fields = row.moduleId ? MODULE_FIELDS[row.moduleId as ModuleId] || [] : [];
            ...
            return (
              <div key={row.id} className="space-y-1.5 p-2.5 rounded-md border border-border bg-background">
                {idx > 0 && (
                  <div className="text-[10px] text-center text-muted-foreground font-medium -mt-1 mb-1">
                    {logic}
                  </div>
                )}
                ...
                <Select
                  value={row.moduleId}
                  onValueChange={(v) =>
                    updateRow(row.id, { moduleId: v as ModuleId, fieldId: '', operator: 'eq', value: '', value2: '' })
                  }
                >
                ...
                {row.moduleId && (
                  <Select
                    ...
                    onValueChange={(v) => {
                      ...
                      const defaultOp = fd ? OPERATORS_BY_TYPE[fd.type][0]?.id || 'eq' : 'eq';
                      updateRow(row.id, { fieldId: v, operator: defaultOp, value: '', value2: '' });
                    }}
                  >
                    <SelectTrigger className="h-7 text-xs">
                ...
                {fieldDef && (
                  <div className="flex gap-1.5">
                    <Select
                      value={row.operator}
                      onValueChange={(v) => updateRow(row.id, { operator: v as any })}
                    >
      ...
      <div className="space-y-1.5">
        <Button onClick={onSearch} size="sm" className="w-full h-8 text-xs" disabled={conditions.length === 0}>
          執行條件查詢
        </Button>
