# legacy-data-etl Specification

## Purpose

This capability defines the one-shot ETL pipeline that absorbs the legacy 1.0 system (Access `gene.mdb` plus the SQL Server `DBGEN` database) and the legacy 2.0 system (three MySQL databases) into the new gimc PostgreSQL backend. It specifies the `backend/etl/` layout (orchestrator, pgloader configs, mdbtools/pyodbc extractors, transform/load/verify scripts), the BLOB-on-filesystem materialization layout under `/srv/gimc/blobs/`, the deterministic UUID rules that merge the 1.0 `ptinfo`/Chinese `patient` tables with the 2.0 `main.patient` table without duplicates, the schema-extension carve-outs needed for 1.0-only data (new `main.gcms` table for `GCDATA`, widened `main.gag` for the `MPSUDATA` panel, the `ref` schema for lookup tables), and the verify suite contract that proves DB-mode answers match mock-mode answers. The capability is one-shot: its lifecycle ends after the MVP-3 cutover.
